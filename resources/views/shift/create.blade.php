...
@section('content')
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-xl font-extrabold text-gray-900">Create New Shift</h1>
    </div>
    ...
    <form action="{{ route('shift.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50" placeholder="Enter shift name">
        </div>

        <div>
            <label for="shift_start" class="block text-sm font-medium text-gray-700">Start Time</label>
            <input type="time" id="shift_start" name="shift_start" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50">
        </div>

        <div>
            <label for="shift_end" class="block text-sm font-medium text-gray-700">End Time</label>
            <input type="time" id="shift_end" name="shift_end" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50">
        </div>

        <div>
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Create Shift</button>
