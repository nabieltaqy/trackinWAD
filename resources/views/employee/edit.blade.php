...
<div class="container mt-5">
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-xl font-extrabold text-gray-900 ml-2">Edit Karyawan</h1>
    </div>

    <form action="{{ route('employee.update', $employee->id) }}" method="GET" class="space-y-4">
        @csrf
        @method('PUT')
        
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700">Nama</label>
            <input type="text" id="name" name="name" value="{{ $employee->name }}" required placeholder="Masukkan nama karyawan" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50">
        </div>

        <div>
            <label for="no_telp" class="block text-sm font-medium text-gray-700">No Telepon</label>
            <input type="text" id="no_telp" name="no_telp" value="{{ $employee->no_telp }}" required placeholder="Masukkan nomor telepon" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50">
        </div>

        <div>
            <label for="address" class="block text-sm font-medium text-gray-700">Alamat</label>
            <textarea id="address" name="address" required placeholder="Masukkan alamat karyawan" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50">{{ $employee->address }}</textarea>
        </div>

        <div class="d-flex justify-content-between mt-4">
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Update</button>
            <a href="{{ route('employee.index') }}" class="ml-2 text-gray-500">Kembali</a>
        </div>
    </form>
</div>
@endsection
