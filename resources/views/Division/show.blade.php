...
<div class="container mt-5">
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-xl font-extrabold text-gray-900 ml-2">Detail Divisi</h1>
    </div>

    <div class="bg-white p-4 rounded-md shadow">
        <p><strong>ID:</strong> {{ $division->id }}</p>
        <p><strong>Nama Divisi:</strong> {{ $division->name }}</p>
        <p><strong>Deskripsi:</strong> {{ $division->description }}</p>
        
        <div class="mt-4">
            <a href="{{ route('division.index') }}" class="bg-yellow-500 text-white px-4 py-2 rounded">Kembali</a>
        </div>
    </div>
</div>
@endsection
